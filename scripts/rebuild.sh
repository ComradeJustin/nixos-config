...
if [[ -f "$CACHE_KEY" ]] && timeout 5 ssh -o ConnectTimeout=3 -o BatchMode=yes justin@nixpc true 2>/dev/null; then
    ...
    nix copy --to ssh-ng://justin@nixpc "$SYSTEM_PATH" 2>/dev/null \
        && ok "Cache updated!" \
